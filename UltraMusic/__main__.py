...
# 6. Notifies the Telegram logger channel that the bot started successfully
# 7. Keeps the bot running until manually stopped
# ==============================================================================
...
from UltraMusic.helpers._logger_channel import send_log
...
async def main():
    try:
        ...
        # Step 8: Notify logger channel that the bot is up and running.
        # Uses fire-and-forget (create_task) so startup is not blocked if the
        # channel is temporarily unreachable.  Failures are swallowed inside
        # send_log() — see UltraMusic/helpers/_logger_channel.py.
        asyncio.create_task(send_log(
            f"✅ <b>ʙᴏᴛ ꜱᴛᴀʀᴛᴇᴅ</b>\n\n"
            f"<b>ɴᴀᴍᴇ:</b> {app.name}\n"
            f"<b>ᴜꜱᴇʀɴᴀᴍᴇ:</b> @{app.username}\n"
            f"<b>ᴘʟᴜɢɪɴꜱ ʟᴏᴀᴅᴇᴅ:</b> {len(all_modules)}\n"
            f"<b>ꜱᴜᴅᴏ ᴜꜱᴇʀꜱ:</b> {len(app.sudoers)}"
        ))
