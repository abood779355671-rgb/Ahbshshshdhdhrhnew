...
from UltraMusic.helpers import Track, admin_check, buttons, can_manage_vc, thumb
...
async def _controls(_, query: types.CallbackQuery):
    ...
    # Handle show_queue action
    if action == "show_queue":
        return await handle_show_queue(query, chat_id)
    ...
    if action == "pause":
        ...
        if qaction:
            return await query.edit_message_reply_markup(
                reply_markup=buttons.controls(
                    chat_id, playing=False, queue_toggle=True, show_extra=False)
            )
        ...
        if qaction:
            return await query.edit_message_reply_markup(
                reply_markup=buttons.controls(
                    chat_id, playing=True, queue_toggle=True, show_extra=False)
            )
        ...
        current_vol = await db.get_volume(chat_id)
        new_vol = min(200, current_vol + 20)
        ok = await tune.change_volume(chat_id, new_vol)
        status = f"🔊 {new_vol}%" if ok else query.lang["not_playing"]
        reply = f"🔊 الصوت رُفع إلى {new_vol}% بواسطة {user}" if ok else ""
        ...
        ok = await tune.change_volume(chat_id, new_vol)
        status = f"🔉 {new_vol}%" if ok else query.lang["not_playing"]
        reply = f"🔉 الصوت خُفِّض إلى {new_vol}% بواسطة {user}" if ok else ""

    elif action.startswith("seekp_"):
        pct = int(action.split("_")[1])

        media = queue.get_current(chat_id)

        if not media or media.is_live or not media.duration_sec:
            await query.answer("⛔ لا يمكن التقديم في البث المباشر", show_alert=True)
            return

        target = int(media.duration_sec * pct / 100)

        ok = await tune.seek_stream(chat_id, target)

        status = f"⏩ {pct}%" if ok else None
        reply = f"⏩ تم التقديم إلى {pct}% بواسطة {user}" if ok else ""
    ...
    try:
        if action in ["skip", "replay", "stop"]:
            ...
            is_playing = False if action == "pause" else await db.playing(chat_id)
...
async def handle_show_queue(query: types.CallbackQuery, chat_id: int):
    """Handle the 📃 show_queue button - reuses the same queue display logic
    as plugins/playback-controls/queue.py, sent as a new message so the
    original controls message (and its keyboard) is left untouched.
    """
    await query.answer()
    ...
    if not _queue:
        return await query.message.reply_text(
            query.lang["not_playing"], quote=False
        )
    ...
    _text = query.lang["queue_curr"].format(
        _media.url,
        _media.title[:50],
        _media.duration,
        _media.user,
    ...
    if _queue:
        ...
        for i, media in enumerate(_queue, start=1):
            if i == 15:
                break
            _text += query.lang["queue_item"].format(
