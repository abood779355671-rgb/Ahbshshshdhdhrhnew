            ...
            for chat_id in list(db.active_calls):
                try:
                    ...
                    if not media:
                        continue
                    media.time += 1
                    if media.time % 30 == 0 and not media.is_live:
                        try:
                            await db.save_last_played(chat_id, {"last_time": media.time})
                        except Exception:
                            pass
                except Exception as e:
            ...
            try:
                ...
                await app.edit_message_reply_markup(
                    ...
                    reply_markup=buttons.controls(
                        chat_id=chat_id, timer=timer_text, remove=remove, playing=True),
                )
            except Exception as e:
                error_str = str(e)
