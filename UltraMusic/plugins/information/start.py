...
from UltraMusic.helpers import buttons, command, start_image, utils


@app.on_message(command(["مساعدة"]) & filters.private & ~app.bl_users)
@lang.language()
...
# NOTE: "start" is kept here on purpose — Telegram's Start button and deep-links
# (referral links) always send the literal "/start", so removing it would break
# them. The Arabic "بدء" trigger is added alongside it.
@app.on_message(command(["بدء", "start"]))
@lang.language()
async def start(_, message: types.Message):
    ...
    # Pick the /start photo source: locally-generated image when explicitly
    # enabled via config.USE_GENERATED_START_IMG, external config.START_IMG
    # otherwise (unchanged default behavior). getattr(..., False) is kept as
    # a safety net for any deployment still running an older config.py that
    # predates this flag — same defensive pattern already used for
    # DATABASE_MODE in UltraMusic/__init__.py.
    photo = config.START_IMG
    if getattr(config, "USE_GENERATED_START_IMG", False):
        try:
            generated = await start_image.generate(app.name)
            if generated:
                photo = generated
        except Exception:
            pass  # keep config.START_IMG as the safe fallback
